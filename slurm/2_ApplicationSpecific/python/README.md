...
The provided [serial] example is of a Python program [fibonacci.py](./serial/fibonacci.py) with the corresponding Slurm script [python-sp.bash](./serial/python-sp.bash) that can be modified to run a serial Python job.
...
The following line in our example script [fibonacci_joblib.py](./parallel./fibonacci_joblib.py) shows how to apply the function to compute fibonacci numbers across an array of input values:
...
For a more in depth discussion on `joblib`, see the [Joblib documentation](https://joblib.readthedocs.io/en/stable/).
